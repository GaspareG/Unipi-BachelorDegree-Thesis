...
We restrict our attention to $q = O(log |V|)$ and denote with $\chi : V \rightarrow [q]$\footnote{Where $[k] \equiv [1, \ldots, q]$ is the set of all possible colors} the coloring function, where each node $u \in V$ have a color $\chi(u) \in [q]$.\\
