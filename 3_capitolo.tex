...
\begin{lemma}
	The execution of \textsc{Bray-Curtis} or \textsc{Frequency-Jaccard} require $O(|W|)$ time and $O(1)$ space. 	
\end{lemma}
...
\section{Naive approach}
	...
	\begin{lemma}
		For any two sets of nodes $A, B \subseteq V$, the running time of \textsc{brute-force} is $O(|V|^{q})$ time and require $O(\mathcal{L}) = O(|\Sigma|^{q})$ space.
	\end{lemma}
