...
In Algorithms 9 TODO 

% replace both Algorithm 6 and 8 by simply add the lines $8$ and $9$ to the Algorithm 6
to compute the frequency map $f_{X}[\langle u_{q}, x \rangle]$ based on sampled $q$-paths $R$.\\
...
To estimate the Bray-Curtis index we call \fsamp with $X = A \uplus B$.
...
We have shown how to estimate the two similarity indices seen using the two approaches \fcont and \fsamp,
