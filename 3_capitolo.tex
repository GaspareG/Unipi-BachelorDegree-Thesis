...
\section{Naive approach}
	...
	When the path $\pi$ is not completed we visit all its neighbor.\\
	...
	\begin{minipage}{0.7\textwidth}\raggedright
		\begin{esempio}
		\end{esempio}
		We want to compute the similarity between the two nodes $4$ and $3$.\\
		...
		And the similarity according to the two indices are:\\
		$BC(\{3\}, \{4\}) = $\\
		$FJ(\{3\}, \{4\}) = $\\
